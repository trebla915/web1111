...
import { z } from 'zod';
import { GREETING_INSTRUCTIONS, VOICE_INSTRUCTIONS } from './instructions.ts';
...
async function callVenueApi<T>(action: string, fields: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!apiBaseUrl || !apiSecret) throw new Error('The reservation system is unavailable.');
...
type EventsResponse = { events: { id: string; title: string; date: string; reservationsAvailable: boolean }[] };
type AvailabilityResponse = {
  event: { id: string; title?: string; date?: string };
  tables: { id: string; number: number; capacity: number; price: number; minimumBottles: number; available: boolean; location: string }[];
};
type BottlesResponse = { bottles: { id: string; name: string; price: number }[] };
type QuoteResponse = {
  event: { id: string; title?: string; date?: string };
  table: { id: string; number: number; capacity: number; minimumBottles: number };
  guestCount: number;
...
type CheckoutResponse = { sent: boolean; expiresAt: string; quote: { tableNumber: number; total: number } };

type CallState = {
  /** Raw caller ID; never placed in a tool result or log. */
  callerNumber?: string;
  /** The number the latest verification code went to, raw caller ID or as the caller gave it. */
  codeSentTo: string;
  verified: boolean;
  verifiedReservations: unknown[];
};
...
export class VenueAgent extends voice.Agent {
  ...
  constructor(instructions: string = VOICE_INSTRUCTIONS) {
    const call: CallState = { codeSentTo: '', verified: false, verifiedReservations: [] };
    ...
    const listEvents = llm.tool({
      ...
      parameters: z.object({}),
      execute: async () => {
        const { events } = await callVenueApi<EventsResponse>('events');
    ...
    const checkAvailability = llm.tool({
      ...
      execute: async ({ eventId }) => {
        const { event, tables } = await callVenueApi<AvailabilityResponse>('availability', { eventId });
    ...
    const listBottles = llm.tool({
      ...
      execute: async ({ eventId, ageConfirmed }) => {
        const { bottles } = await callVenueApi<BottlesResponse>('bottles', { eventId, ageConfirmed });
    ...
    const quoteReservation = llm.tool({
      ...
      execute: async ({ eventId, tableId, guestCount, bottleIds }) => {
        const quote = await callVenueApi<QuoteResponse>('quote', { eventId, tableId, guestCount, bottleIds });
        return {
    ...
    const checkCallerId = llm.tool({
      ...
      parameters: z.object({}),
      execute: async () => {
        if (!call.callerNumber) return { callerIdAvailable: false };
        try {
          const { hasPossibleReservation } = await callVenueApi<{ hasPossibleReservation: boolean }>('caller-lookup', { phone: call.callerNumber });
    ...
    const sendCodeToCaller = llm.tool({
      ...
      execute: async () => {
        if (!call.callerNumber) return { sent: false, message: 'Caller ID is unavailable. Ask for the phone number on the reservation.' };
    ...
    const verifyLookupCode = llm.tool({
      ...
      execute: async ({ code }) => {
        if (!call.codeSentTo) return { verified: false, message: 'No code has been sent yet. Do not disclose reservation details.' };
        const result = await callVenueApi<{ verified: boolean; reservations?: unknown[] }>('verify-lookup-code', { phone: call.codeSentTo, code });
    ...
    const createCheckout = llm.tool({
      ...
      execute: async ({ eventId, tableId, guestCount, bottleIds, name, phone, email, ageConfirmed, smsConsent, callerConfirmed, quotedTotal }) => {
        ...
        const result = await callVenueApi<CheckoutResponse>('create-checkout', { eventId, tableId, guestCount, bottleIds, name, phone, email, ageConfirmed, smsConsent, callerConfirmed, quotedTotal });
  ...
  setCallerNumber(number: string | undefined): void {
