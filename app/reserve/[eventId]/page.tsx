...
import { fetchTablesByEvent } from '@/lib/services/tables';
    ...
    try {
      ...
      if (event) {
        setEventDetails({
          ...
          imageUrl: event.flyerUrl || "/images/event-placeholder.jpg"
        });
      } else {
      ...
      const fetchedTables = await fetchTablesByEvent(eventId);
      ...
      const formattedTables = fetchedTables.map(table => ({
        ...table,
        location: String(table.location || '').toLowerCase(),
      }));
